...
ConVar	sk_Houndeye_dmg_blast( "sk_Houndeye_dmg_blast","20");

int AE_HOUND_RANGE_ATTACK1;
int AE_HOUND_PRE_RANGE_ATTACK1;
...
void CNPC_Houndeye::InitCustomSchedules(void) 
{
	...
	DECLARE_ANIMEVENT( AE_HOUND_RANGE_ATTACK1 )
	DECLARE_ANIMEVENT( AE_HOUND_PRE_RANGE_ATTACK1 )
}

LINK_ENTITY_TO_CLASS( npc_houndeye, CNPC_Houndeye );
//IMPLEMENT_SERVERCLASS_ST( CNPC_Houndeye, DT_NPC_Houndeye )
//END_SEND_TABLE()
...
{
	int vent = pEvent->event;
	if(vent == AE_HOUND_RANGE_ATTACK1){
		Msg("HoundEye attack\n");
		SonicAttack();
	}
	
	if(vent == AE_HOUND_PRE_RANGE_ATTACK1){
		Msg("HoundEye Pre attack\n");
...
void CNPC_Houndeye::Precache()
{
	...
	PrecacheParticleSystem( "houndeye_sonic_attack" );
...
void CNPC_Houndeye::SonicAttack ( void )
{
	...
	DispatchParticleEffect( "houndeye_sonic_attack", this->GetAbsOrigin(), this->GetAbsAngles());
...
void CNPC_Houndeye::StartTask( const Task_t *pTask )
{
	...
	case TASK_HOUND_GET_PATH_TO_CIRCLE:
	{
		...
		else 
		{
			...
			/*if (GetNavigator()->SetRadialGoal(vTargetPos, RandomVector(50,500), 90, 175, m_bLoopClockwise, false))
			{
				TaskComplete();
				return;
			}
			TaskFail(FAIL_NO_ROUTE);*/
			if(GetNavigator()->SetVectorGoalFromTarget( GetEnemy()->GetLocalOrigin() )){
				TaskComplete();
			}
			return;
		}
